...
void QPropertyModel::addItem(QObject *propertyObject)
{
    ...
    propertyItem = new Property(/*name*/"General", 0, m_rootItem);
    foreach(PropertyPair pair, propertyMap)
    {
        ...
        if( p == 0)
        {
            uint classinfo_count = propertyObject->metaObject()->classInfoCount();
            bool forced_type = false;
            for(uint i = 0; i < classinfo_count; i++)
            {
                QMetaClassInfo info = propertyObject->metaObject()->classInfo(i);
                if(strcmp(info.name(), property.name()) == 0)
                {
                    std::vector<std::string> items = StringHelper::split(info.value(), "=");
                    std::string var = items[0];
                    std::string var_value = items[1];
                    if(var == "CustomEnum")
                    {
                        if(var_value == "CClasses")
                        {
                            p = new CObjectTypesEnumProperty(property.name(), propertyObject, propertyItem);
                            auto types = CGameObjectFactory::types();
                            ((CObjectTypesEnumProperty*)p)->setEnumValues(types);
                            forced_type = true;
                        }
                    }
                }
