...
QVariantDelegate::QVariantDelegate(QObject* parent) : QItemDelegate(parent)
{
    ...
    connect(this, SIGNAL(beginPropertyEdit()),
            EditorWindow::currentInstance, SLOT(onEditPropertyBegin()));
    connect(this, SIGNAL(finishPropertyEdit()),
            EditorWindow::currentInstance, SLOT(onEditPropertyEnd()));
}
...
QWidget *QVariantDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem& option , const QModelIndex & index ) const
{
    ...
    // fix needed for boolean Properties
    if (editor->metaObject()->indexOfSignal("currentIndexChanged(int)") != -1)
    {
        connect(editor, SIGNAL(currentIndexChanged(int)), m_finishedMapper, SLOT(map()));
		m_finishedMapper->setMapping(editor, editor);
    }
    // --------
    connect(editor, SIGNAL(destroyed(QObject*)),
            this, SLOT(editorDestroyed(QObject*)));
    emit beginPropertyEdit();
	return editor;
}
...
void QVariantDelegate::editorDestroyed(QObject*)
{
