...
QVariant Vec3fProperty::value(int role) const
{
    ...
    if (data.isValid() && role != Qt::UserRole)
    {
        ...
        {
        case Qt::DisplayRole:
            return tr("[ %1, %2, %3]").arg(data.value<btVector3>().x()).arg(data.value<btVector3>().y()).arg(data.value<btVector3>().z());
        case Qt::EditRole:
            return tr("%1, %2, %3").arg(data.value<btVector3>().x()).arg(data.value<btVector3>().y()).arg(data.value<btVector3>().z());
        };
    }
...
void Vec3fProperty::setValue(const QVariant& value)
{
	if (value.type() == QVariant::String)
	{
		...
		m_y->setProperty("y", y);
		m_z->setProperty("z", z);
        Property::setValue(QVariant::fromValue(btVector3(x, y, z)));
	}
	else
...
{
    return value().value<btVector3>().x();
}
...
{
    return value().value<btVector3>().y();
}
...
{
    return value().value<btVector3>().z();
}
