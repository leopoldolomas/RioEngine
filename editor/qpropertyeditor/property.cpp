...
m_propertyObject(propertyObject)
{
    ...
    if(EditorWindow::currentInstance)
    {
        connect(this, SIGNAL(propertyChanged(QObject*,QString,QVariant,QVariant)),
                EditorWindow::currentInstance, SLOT(propertyChanged(QObject*,QString,QVariant,QVariant)));
    }
}
...
void Property::setValue(const QVariant &value)
{
	if (m_propertyObject)
    {
        QVariant old_value = m_propertyObject->property(qPrintable(objectName()));
		m_propertyObject->setProperty(qPrintable(objectName()), value);
        emit propertyChanged(m_propertyObject, objectName(), value, old_value);
    }
}
