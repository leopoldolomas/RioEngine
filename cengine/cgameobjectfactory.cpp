...
QStringList CGameObjectFactory::m_types;

//-----------------------------------------------------------------------------

CGameObject* CGameObjectFactory::createInstance(const char* class_name) {
    if(!m_types.contains(QString::fromUtf8(class_name))) {
        LOG("%s has not been added to CGameObjectFactory", class_name);
        RE_ASSERT(false);
        return NULL;
    }
    int type = QMetaType::type(class_name);
    CGameObject* new_object = static_cast<CGameObject*>(QMetaType::create(type));
    new_object->autorelease();
    return new_object;
}
...
int CGameObjectFactory::getObjectTypeId(const char* class_name) {
    return m_types.indexOf(QString::fromUtf8(class_name));
}
